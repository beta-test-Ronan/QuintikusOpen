...
class QuintikusAGI:
    def __init__(self, _t=0.5):
        ...
        # Mapa Térmico Minimalista (Peso Positivo, Peso Negativo)
    ...
    def inicializar(self, _txt):
        """SNAPSHOT TDLM: Mapeamento de Massa Crítica"""
        ...
        for i in range(0, len(_s), 5):
            ...
            for _st in _bk:
                _p = sorted(_st.split(), key=lambda x: self._m.get(x, (0,0))[1], reverse=True)[0]
    ...
    def _upd_thermal(self, _q):
        ...
        # T = T * 0.95 + (N * 0.1) | S = S * 0.95 + (P * 0.1)
        self._st[0] = self._st[0] * 0.95 + (_n * 0.1)
        self._st[1] = self._st[1] * 0.95 + (_p * 0.1)
        for i in range(3): self._st[i] = max(0, min(1, self._st[i]))

    def falar(self, _qi):
        """DLM-FLOW v128: Síntese de Emoção e Lógica"""
        _t0 = time.perf_counter()
        _ql = _qi.lower().replace("?", "").split()
        _qs = set(_ql)
        self._upd_thermal(_ql) # Sente o calor do input
        
        # O Dado de Hummer agora é influenciado pela Temperatura
        _dice = random.randint(1, 10)
        _bias = 2 if self._st[0] > 0.6 else -2 # Se está "quente", tende ao Marvin
        ...
        if _me and _br > 0:
            ...
            _v_sc = sum(self._m.get(w,(0,1))[0] / (self._m.get(w,(0,1))[1] + 1e-5) for w in _qs if w in _f1)
            _nxt = self._dlm.get(_me["id"])
            _f2 = next((x["b"][0] for x in self._e if x["id"] == _nxt), "")
            _sn = f"DLM-ACTIVE|V:{_v_sc:.1f}"
        else:
            _pv = next((w for w in _ql if w in self._c), None)
            _f1, _f2, _sn = (self._c[_pv][0], "", "PIVOT") if _pv else (f"o nexo '{_ql[:1]}' carece de solo", random.choice(self._l), "VOID")
        ...
        _st_info = f"T:{self._st[0]:.1f}|S:{self._st[1]:.1f}"
        return f"\n[DLM-FLOW: {_et:.2f}μs | D:{_final_d}/10 | {_st_info} | {_sn} | SIGN: {self._s}]\n{_res.capitalize()}"
...
print(quantikus.falar(" "))



def efeito_llm(texto, velocidade=5.0):
