...
class Quintikus:
    def __init__(self, raw_text=None, d_model=512000, model_path="quintikus_v41.model"):
        self.model_path = model_path
        self.ethos_tension = 0.5
        self.chains = {} 
        self.word_rarity = {} 
        self.arquétipos = {}
    ...
    def inicializar(self, texto):
        self.sentences = [s.strip() for s in texto.lower().split('.') if len(s.strip()) > 5]
        all_words = " ".join(self.sentences).split()
        for w in set(all_words):
            self.word_rarity[w] = 1.5 / (all_words.count(w) + 1e-5)
        
        for sent in self.sentences:
            h = hashlib.sha256(sent.encode()).hexdigest()[:8]
            # Pega a palavra mais rara do fato como âncora
            pivo = sorted(sent.split(), key=lambda x: self.word_rarity.get(x, 0), reverse=True)[0]
            if pivo not in self.chains: self.chains[pivo] = []
            self.chains[pivo].append({"fato": sent, "hash": h})
        
        self.model_signature = hashlib.sha256(str(list(self.chains.keys())).encode()).hexdigest()[:8]
        for m in ['mc', 'mm', 'mn']:
            self.arquétipos[m] = self._carregar_vozes(f'{m}.txt')

    def falar(self, pergunta):
        """O Fim da Roboticidade: Fusão Narrativa Natural"""
        start_t = time.perf_counter()
        q_words = pergunta.lower().replace("?", "").split()
        
        # 1. MINERAÇÃO TRANS-CADEIA (Igual v40)
        pool = [f for c in self.chains.values() for f in c]
        scores = sorted([(sum(self.word_rarity.get(qw, 0) * 5000 for qw in q_words if qw in f['fato'].split()), f) for f in pool], key=lambda x: x[0], reverse=True)
        
        if not scores or scores[0][0] < 5:
            return "Meus circuitos de nexo indicam um vácuo de informação. Deprimente, eu diria."

        # 2. SELEÇÃO DE EVIDÊNCIA (Fato 1 e Fato 2)
        f1 = scores[0][1]['fato']
        f2 = scores[1][1]['fato'] if len(scores) > 1 and scores[1][1]['fato'] != f1 else ""

        # 3. PROJEÇÃO TEÓRICA (Tier 3)
        evolucao = [s[1]['fato'] for s in scores if any(w in s[1]['fato'] for w in ["pacto", "aceitou", "respeito", "agora", "diálogo"])]
        teoria = f" Tudo indica que a trajetória converge para {evolucao[0]}." if evolucao else ""

        # 4. A VOZ DO SÁBIO (C-3PO + MARVIN)
        # Sorteamos um 'Tom' para a resposta inteira
        tom = random.choice(['mc', 'mm']) 
        v = self.arquétipos[tom]
        


        intro = random.choice(v['intro']).strip()
        ponte = random.choice(v['ponte']).strip()
        concl = random.choice(v['concl']).strip()

        # Construção da frase como um pensamento contínuo
        narrativa = f"{intro} {f1}"
        if f2:
            narrativa += f", e ao analisar os elos secundários, percebo que {f2}"
        
        if teoria:
            narrativa += f". {ponte} {teoria}"
        
        # Fechamento com autoridade e peso existencial
        paragrafo_final = f"{narrativa}. {concl} (Assinado: {self.model_signature})."

        exec_time = (time.perf_counter() - start_t) * 1000000
        return f"\n[PENSAMENTO LINEAR: {exec_time:.2f} μs]\n{paragrafo_final.capitalize().replace(' .', '.')}"

    def _carregar_vozes(self, path):
        if not os.path.exists(path): return {"intro":[" "],"ponte":[","],"concl":[" "]}
...
# --- BANCO DE DADOS ---
banco = """
A Cartomante o livro de Machado de Assis
...
telhado dos fundos. Velhos trastes, paredes sombrias, um ar de pobreza, que antes
...
e, com dois tiros de revólver, estirou-o morto no chão.FIM
"""
with open('model.relacional.txt', 'r', encoding='utf-8') as arquivo:
    conteudo = arquivo.read()
    
#quantikus.inicializar("dados para pensar ->  "+conteudo)
quantikus = Quintikus()
print("inicializar 1 treino") 
quantikus.inicializar(banco)
...
print(quantikus.falar(""""Com base exclusivamente na sua leitura silenciosa do texto 'A Cartomante', d
#escreva o que estava acontecendo atrás da cortina da casa da cartomante enquanto Camilo olhava para as cartas. Você não pode citar Machado. Você só pode descrever o que o seu algoritmo de atenção focou como 'sinal' em meio ao 'ruído' da narrativa.
#Justifique por que seu gradiente descendente parou nesse mínimo local e não em outro."""))

print("loop inicial") 
loop=quantikus.falar("""
...
print("-"*100)
print("loop 1") 
loop1=quantikus.falar(loop)
print(loop1)
print("-"*100)
print("loop 2") 
loop2=quantikus.falar(loop1)
print(loop2)
print("-"*100)
print("loop 3") 
loop3=quantikus.falar(loop+loop1+loop2)
"""

#print(quantikus.falar("Se você soubesse como eu tenho andado, por sua causa... analisei o 'Caminho de Ouro' de Camilo e percebi o erro sistêmico. "))
def efeito_llm(texto, velocidade=1.0):
    for caractere in texto:
        # Imprime o caractere sem pular linha
        print(caractere, end='', flush=True)
        # Pequena pausa para simular o processamento
        time.sleep(velocidade)
    print() # Linha nova ao terminar
